...
namespace BouncyCastleAESAlgorithmCBCMode
{
    public class CryptographicService
    {
        ...
        private byte[] normalizeDataForEncryption(string data) {
            var convertedData = convertDataToBytes(data);
            var paddedData = padData(convertedData);

            return paddedData;
        }
        private byte[] convertDataToBytes(string data)
        {
        ...
        {
            var normalizedDataToEncrypt = normalizeDataForEncryption(dataToEncrypt);
            var encryptionService = setUpEncryptionService(encryptionKey, true);
            
            int encryptionBlockSize = encryptionService.GetBlockSize();
            var cipher = CreateEncryptionHolder(normalizedDataToEncrypt, encryptionBlockSize);

            for (int dataCursor = 0; dataCursor < normalizedDataToEncrypt.Length; dataCursor += encryptionBlockSize) {
                encryptionService.ProcessBlock(normalizedDataToEncrypt, dataCursor, cipher, dataCursor);
            }
        ...
        public string CheckDecryption(byte[] cipherToDecrypt, byte[] encryptionKey)
        {
            ...
            for (int dataCursor = 0; dataCursor < cipherToDecrypt.Length; dataCursor += blockSize)
            {
                encryptionService.ProcessBlock(cipherToDecrypt, dataCursor, plainText, dataCursor);
            }
